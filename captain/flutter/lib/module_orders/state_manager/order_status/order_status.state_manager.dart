...
import 'package:twaslna_captain/module_orders/ui/state/order_status/order_details_state_captain_order_loaded.dart';
...
import 'package:twaslna_captain/module_orders/ui/state/order_status/order_status_loading_state.dart';
import 'package:twaslna_captain/module_orders/ui/state/order_status/orders_status_empty_state.dart';
import 'package:twaslna_captain/module_report/service/report_service.dart';
...
class OrderStatusStateManager {
  ...
  final PublishSubject<OrderDetailsState> _stateSubject =  PublishSubject<OrderDetailsState>();
  ...
  void getOrderDetails(int orderId, OrderStatusScreenState screenState) {
    ...
    _ordersService.getOrderDetails(orderId).then((value){
      if (value.hasError){
        _stateSubject.add(OrderStatusErrorState(screenState,value.error,orderId));
      }
      else if (value.isEmpty){
        _stateSubject.add(OrderDetailsEmptyState(screenState,S.current.homeDataEmpty,orderId));
      }
      else {
        _stateSubject.add(OrderDetailsStateCaptainOrderLoaded(screenState,value.data));
      }
    });
  }

  void updateOrder(UpdateOrderRequest request, OrderStatusScreenState screenState) {
    _stateSubject.add(OrderDetailsStateLoading(screenState));
    _ordersService.updateOrder(request).then((value){
      if (value.hasError) {
        ...
        CustomFlushBarHelper.createSuccess(title:S.current.warnning, message:S.current.updateOrderSuccess)..show(screenState.context);
        getOrderDetails(request.id??-1, screenState);
      }
    });
  }
  void updateInvoice(UpdateOrderRequest orderRequest,OrderInvoiceRequest request, OrderStatusScreenState screenState) {
    _stateSubject.add(OrderDetailsStateLoading(screenState));

...
import 'package:twaslna_captain/module_orders/ui/state/captain_orders/captain_orders_list_state_error.dart';
import 'package:twaslna_captain/module_orders/ui/state/captain_orders/captain_orders_list_state_loading.dart';
import 'package:twaslna_captain/module_orders/ui/state/captain_orders/captain_orders_list_state_orders_loaded.dart';
import 'package:twaslna_captain/module_profile/model/profile_model/profile_model.dart';
import 'package:twaslna_captain/module_profile/service/profile/profile.service.dart';
...
class CaptainOrdersListStateManager {
  ...
  final PublishSubject<CaptainOrdersListState> _stateSubject = PublishSubject<CaptainOrdersListState>();
  final PublishSubject<ProfileModel> _profileSubject = PublishSubject<ProfileModel>();
  final PublishSubject<CompanyInfoResponse> _companySubject = PublishSubject<CompanyInfoResponse>();
  ...
  void getProfile(CaptainOrdersScreenState screenState) {
    _profileService
        ...
        .then((profile) {
          if (profile.hasError){
            CustomFlushBarHelper.createError(title:S.current.warnning, message:profile.error).show(screenState.context);
          }

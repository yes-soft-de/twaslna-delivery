...
import 'package:twaslna_captain/module_orders/response/orders/geojson.dart';
...
class OrderDetailsModel {
  ...
  List<Item> _toCartList(List<OrderDetails> ordersItems) {
    ...
    ordersItems.forEach((element) {
      items.add(Item(
          ...
          productImage: element.productImage ?? Urls.IMAGES_ROOT,
  ...
  OrderInfo _toOrder(Order? order,[LatLng? defaultLoc]) {
    if (order != null) {
      ...
      if (defaultLoc != null) {
        if (sourceLocation != null) sourceDistance = _getDestination(defaultLoc, order.source);
        if (destinationLocation != null) destinationDistance = _getDestination(defaultLoc,order.destination);
      }

      var date = DateTime.fromMillisecondsSinceEpoch(
          (order.deliveryDate?.timestamp ?? DateTime
              .now()
              .millisecondsSinceEpoch) *
              1000).toIso8601String();
      return OrderInfo(
          id: order.id ?? -1,

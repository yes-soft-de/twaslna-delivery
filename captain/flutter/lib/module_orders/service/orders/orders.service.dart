...
import 'package:twaslna_captain/module_deep_links/service/deep_links_service.dart';
import 'package:twaslna_captain/module_orders/manager/orders_manager/orders_manager.dart';
import 'package:twaslna_captain/module_orders/model/order/accept_order.dart';
import 'package:twaslna_captain/module_orders/model/order/action_state_model.dart';
...
import 'package:twaslna_captain/module_orders/response/order_status/order_action_response.dart';
import 'package:twaslna_captain/module_orders/response/order_status/order_status_response.dart';
import 'package:twaslna_captain/module_orders/response/orders/accept_order_response.dart';
import 'package:twaslna_captain/module_orders/response/orders/order_response.dart';
import 'package:twaslna_captain/module_orders/response/orders_logs_response.dart';
import 'package:twaslna_captain/utils/helpers/status_code_helper.dart';
...
//////////////////////////////////////////////////////////////////////
  Future<OrderDetailsModel> getOrderDetails(int orderId) async {
    OrderStatusResponse? _ordersResponse = await _ordersManager.getOrderDetails(orderId);
  ...
  Future<OrderModel> getNearbyOrders() async {
    ...
    if (code != '200') return OrderModel.error(StatusCodeHelper.getStatusCodeMessages(code));
  ...
  Future <ActionStateModel> updateOrder(UpdateOrderRequest request) async {
    OrderActionResponse? actionResponse = await _ordersManager.updateOrder(request);
  ...
  Future <ActionStateModel> updateOrderBill(OrderInvoiceRequest request) async {
    OrderActionResponse? actionResponse = await _ordersManager.updateBill(request);
    if (actionResponse == null) return ActionStateModel.error(S.current.networkError);
    if (actionResponse.statusCode != '204') return ActionStateModel.error(StatusCodeHelper.getStatusCodeMessages(actionResponse.statusCode));
    await insertWatcher();
    return ActionStateModel.empty();
  ...
  Future<AcceptOrderModel> getCaptainOrders() async {
    ...
    String code = response.statusCode.toString();
    if (response.statusCode != '200') return AcceptOrderModel.error(StatusCodeHelper.getStatusCodeMessages(code));

...
import 'package:twaslna_captain/module_orders/ui/widgets/home_widgets/order_card.dart';
...
import 'package:twaslna_captain/utils/components/empty_screen.dart';
import 'package:twaslna_captain/utils/components/error_screen.dart';
...
class CaptainOrdersListStateOrdersLoaded extends CaptainOrdersListState {
  ...
  Widget getUI(BuildContext context) {
    return Scaffold(
      appBar: CustomTwaslnaAppBar.appBar(context, title: S.of(context).home,icon: Icons.sort_rounded,onTap: (){
  ...
  Widget getMyOrdersList(BuildContext context) {
    ...
    if (myOrders.hasError) {
      ...
      return EmptyStateWidget(empty: S.current.homeDataEmpty,
        onRefresh:(){
          screenState.getMyOrders();
        },
      );
      ...
      myOrders.data.forEach((element) {
        uiList.add(OrderCard(
          ...
          orderDate: timeago.format(element.deliveryDate,locale: Localizations.localeOf(context).languageCode),
      ...
      return RefreshIndicator(child:  CustomListView.custom(children: uiList), onRefresh:(){
  ...
  Widget getNearbyOrdersList(BuildContext context) {
    ...
    List<OrderModel> orders = sortLocations(nearbyOrders.data,nearbyOrders.currentLocation);
    var now = DateTime.now();
    var moment = Moment.now();
    var uiList = <Widget>[];
    orders.forEach((element) {
      var orderDate = now.isBefore(element.deliveryDate) ? moment.from(element.deliveryDate) : timeago.format(element.deliveryDate);
      uiList.add(OrderCard(
      orderId: element.orderNumber,

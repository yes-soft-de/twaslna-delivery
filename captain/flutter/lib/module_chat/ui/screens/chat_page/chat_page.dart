...
import 'package:twaslna_captain/module_chat/presistance/chat_hive_helper.dart';
import 'package:twaslna_captain/module_chat/state_manager/chat_state_manager.dart';
import 'package:twaslna_captain/module_chat/ui/screens/chat_state/empty_state.dart';
import 'package:twaslna_captain/module_chat/ui/screens/chat_state/loading_state.dart';
...
class ChatPage extends StatefulWidget {
  ...
  ChatPage(this._chatStateManager, this._uploadService, this._authService,this._chatHiveHelper);
...
class ChatPageState extends State<ChatPage> with WidgetsBindingObserver {
  ...
  streamSubscription =  widget._chatStateManager.chatBlocStream.listen((event) {
      currentState = event.first;
      if (event.first == ChatStateManager.STATUS_CODE_GOT_DATA) {
        _chatMessagesList = event.last;
        if (chatsMessagesWidgets.length - (lastSeenIndex != null ? 1 : 0)  == _chatMessagesList.length) {
        } else {
    ...
    chatScrollController.addListener(() {
      if (lastSeenIndex != null && empty == false) {
        if (!chatScrollController.isIndexStateInLayoutRange(lastSeenIndex!) || chatScrollController.offset != widget._chatHiveHelper.getChatOffset(chatRoomId, widget._authService.username)){
          widget._chatHiveHelper.deleteChatCache(chatRoomId + widget._authService.username);
          setState(() {
          });
        }
      }
      if (!chatScrollController.isIndexStateInLayoutRange(chatsMessagesWidgets.length - 1) && empty == false) {
        down = true;
        if (mounted) {
          setState(() {});
        }
      }
      if (chatScrollController.isIndexStateInLayoutRange(chatsMessagesWidgets.length - 1) && down) {
        down = false;
        if (mounted) {}
        setState(() {});
      }
  ...
  Widget build(BuildContext context) {
    ...
    if (currentState == ChatStateManager.STATUS_CODE_EMPTY_LIST) {
      return EmptyChatPage(widget._chatStateManager,widget._uploadService,widget._authService);
    } else if (currentState == ChatStateManager.STATUS_CODE_GOT_DATA &&
        chatsMessagesWidgets.isEmpty) {
      return LoadingChatPage(widget._chatStateManager,widget._uploadService,widget._authService);
    }
    return WillPopScope(
      onWillPop: () async {
        int remove = 0;
        ...
        widget._chatHiveHelper.setChatIndex(chatRoomId,widget._authService.username,chatsMessagesWidgets.length - remove);
        widget._chatHiveHelper.setChatOffset(chatRoomId,widget._authService.username,chatScrollController.offset);
        return true;
      },
      child: GestureDetector(
        ...
        child: Scaffold(
            appBar:
                CustomTwaslnaAppBar.appBar(context, title: S.current.chatRoom,onTap: (){
                  int remove = 0;
                  if (newMessagesWidgetExist()) {
                    remove = 1;
                  }
                    ...
                    ChatWriterWidget(
                      onTap: () {
                        if (lastSeenIndex!=null || newMessagesWidgetExist()) {
                          widget._chatHiveHelper.deleteChatCache(chatRoomId + widget._authService.username).whenComplete((){
                            lastSeenIndex = null;
                            widget._chatStateManager.getMessages(chatRoomId);
                          });
                        }
                          ...
                          child: ScalingWidget(
                            child: Card(
                              ...
                              child: InkWell(
                                onTap: () {
                                  down = false;
                                  chatScrollController.scrollToIndex(
                                      chatsMessagesWidgets.length - 1).whenComplete((){
                                        setState(() {
                                        });
                                  });
                                  setState(() {
  ...
  Future<void> buildMessagesList(List<ChatModel> chatList) async {
    ...
    bool newMessages = lastSeenIndex!=null ? (lastSeenIndex! < chatList.length) : false;
        ...
        child: Container(
          ...
          child: Padding(
            ...
            child: Container(
              ...
              child: Padding(
                padding: const EdgeInsets.all(16.0),
                child: Text(S.current.lastSeenMessage,style: TextStyle(

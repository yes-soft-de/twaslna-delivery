...
import 'package:twaslna_dashboard/module_captain/ui/state/captains_payments/captains_payments_loaded_state.dart';
...
class CaptainsPaymentsStateManager {
  ...
  void getCaptains(CaptainsPaymentsScreenState screenState){
    ...
    _captainsService.getPayments().then((value){
      if (value.hasError){
        _stateSubject.add(CaptainsPaymentsLoadedState(screenState,null,error: value.error));
      }
      else if (value.isEmpty) {
        _stateSubject.add(CaptainsPaymentsLoadedState(screenState,null,empty: value.isEmpty));
      }
      else {
        CaptainPaymentModel _model = value as CaptainPaymentModel;
        _stateSubject.add(CaptainsPaymentsLoadedState(screenState,_model.data));
      }
    });
  }

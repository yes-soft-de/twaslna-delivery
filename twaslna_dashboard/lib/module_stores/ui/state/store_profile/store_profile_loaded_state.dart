...
class StoreProfileLoadedState extends States {
  ...
  final String? error;
  final bool empty;
  StoreProfileLoadedState(this.screenState,this.profile,{this.error,this.empty = false}) : super(screenState);
  ...
  Widget getUI(BuildContext context) {
    ...
    return CustomListView.custom(children:[
      ...
      CustomListTile(title:S.current.storeName ,subTitle:profile?.storeOwnerName,),
      CustomListTile(title:S.current.categoryName ,subTitle:screenState.model?.categoryId,),
      CustomListTile(title:S.current.storePhone ,subTitle:profile?.phone,),
      CustomListTile(title:S.current.deliverPrice ,subTitle:'${screenState.model?.deliveryCost.toString() ?? 0} ${S.current.sar}',),
      CustomListTile(title:S.current.products ,serve:profile?.hasProducts),
      CustomListTile(title:S.current.privateOrder ,serve:profile?.privateOrders),
      Container(
        width:double.maxFinite,
        color: Theme.of(context).backgroundColor,
        child: Padding(

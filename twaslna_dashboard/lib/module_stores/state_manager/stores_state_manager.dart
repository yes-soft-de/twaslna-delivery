...
class StoresStateManager {
  ...
  StoresStateManager(this._storesService, this._authService,this._uploadService,this._categoriesService);
  ...
  void getStores(StoresScreenState screenState) {
    ...
    _categoriesService.getStoreCategories().then((categories){
      if (categories.hasError){
        _stateSubject.add(StoresLoadedState(screenState,null,null,error: categories.error));
      }
      else if (categories.isEmpty) {
        _stateSubject.add(StoresLoadedState(screenState,null,null,empty: categories.isEmpty));
      }
      else {
        _storesService.getStores().then((value) {
          if (value.hasError) {
            _stateSubject.add(StoresLoadedState(screenState,null,null,error: value.error));
          } else if (value.isEmpty) {
            StoreCategoriesModel categoriesModel = categories as StoreCategoriesModel;
            _stateSubject.add(StoresLoadedState(screenState,[],categoriesModel.data));
          } else {
            StoresModel model = value as StoresModel;
            StoreCategoriesModel categoriesModel = categories as StoreCategoriesModel;
            _stateSubject.add(StoresLoadedState(screenState,model.data,categoriesModel.data));
          }
        });
      }
    });
  ...
  void createCategory(StoresScreenState screenState,CreateStoreRequest request){

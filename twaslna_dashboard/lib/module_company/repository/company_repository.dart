...
class CompanyRepository {
  ...
  Future<ActionResponse?> updateFinancialCompany(FinancialCompensationRequest request,bool create) async {
    var token = await _authService.getToken();
    dynamic response;
    if (create){
      response = await _apiClient.post(Urls.UPDATE_FINANCIAL_COMPANY,request.toJson(),
          headers: {'Authorization': 'Bearer ' + token.toString()});
    }
    else {
      response = await _apiClient.put(Urls.UPDATE_FINANCIAL_COMPANY,request.toJson(),
          headers: {'Authorization': 'Bearer ' + token.toString()});
    }
    return ActionResponse.fromJson(response);
  }
  Future<ActionResponse?> updateFinancialDeliveryPriceCompany(DeliveryCompanyFinancialRequest request,bool create) async {
    var token = await _authService.getToken();
    dynamic response;
    if (create){
      response = await _apiClient.post(Urls.UPDATE_DELIVERY_PRICE_COMPANY,request.toJson(),
          headers: {'Authorization': 'Bearer ' + token.toString()});
    }
    else {
      response = await _apiClient.put(Urls.UPDATE_DELIVERY_PRICE_COMPANY,request.toJson(),
          headers: {'Authorization': 'Bearer ' + token.toString()});
    }
    if (response == null) return null;
    return ActionResponse.fromJson(response);

...
class CompanyFinanceLoadedState extends States {
  ...
  CompanyFinanceLoadedState(this.screenState,{this.financeModel,required this.model,
      this.empty = false, this.error,this.errors})
      : super(screenState) {
    if (error != null) {
      screenState.refresh();
      CustomFlushBarHelper.createError(title: S.current.warnning, message: error??'');
    }
    if (errors != null){
      screenState.refresh();
    }
    if (financeModel != null) {
      ...
      maxKilometerBonusController.text = financeModel?.maxKilometerBonus.toString() ?? '';
      minKilometerBonusController.text = financeModel?.minKilometerBonus.toString() ?? '';
      screenState.refresh();
    }
    if (model != null) {
    ...
    return StackedForm(
        ...
        onTap: () {
          if (_key.currentState!.validate()) {
              screenState.updateFinancial(FinancialCompensationRequest(
                  id:financeModel?.id,
                  kilometers:int.parse(kilometerController.text.trim()),
                  maxKilometerBonus:double.parse(maxKilometerBonusController.text.trim()),
                  minKilometerBonus:double.parse(minKilometerBonusController.text.trim()),
              ),

...
import 'package:twaslna_dashboard/module_company/ui/state/company_profile/company_loaded_state.dart';
...
class CompanyProfileStateManager {
  ...
  void getCompanyProfile(CompanyProfileScreenState screenState) {
    ...
    _companyService.getCompanyProfile().then((value) {
      if (value.isEmpty){
        ...
        _stateSubject.add(CompanyLoadedState(screenState, null,error:value.error));
      }
      else {
        CompanyProfileModel model = value as CompanyProfileModel;
        _stateSubject.add(CompanyLoadedState(screenState, model.data,error:value.error));
      }
    });
  }

  void createProfile(CompanyProfileScreenState screenState,CreateCompanyProfile request){
  ...
  void updateStore(CompanyProfileScreenState screenState,CreateCompanyProfile request){

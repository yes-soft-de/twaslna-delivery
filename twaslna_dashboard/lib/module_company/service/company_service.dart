...
import 'package:twaslna_dashboard/module_company/response/financial_compensations_response.dart';
...
class CompanyService {
  ...
  Future<DataModel> getFinancialCompany() async {
    FinancialCompensationsResponse? _companyResponse = await _companyManager.getFinancialCompany();
    if (_companyResponse == null) {
      return DataModel.withError(S.current.networkError);
    }
    if (_companyResponse.statusCode != '200') {
  ...
  Future<DataModel> getDeliveryPrice() async {
    CompanyDeliveryPriceResponse? _companyResponse = await _companyManager.getDeliveryPrice();
    if (_companyResponse == null) {
      return DataModel.withError(S.current.networkError);
    }
    if (_companyResponse.statusCode != '200') {
  ...
  Future<DataModel> createCompanyFinancial(FinancialCompensationRequest request) async {
    ActionResponse? actionResponse = await _companyManager.createFinancialCompany(request);
  ...
  Future<DataModel> updateCompanyFinancial(FinancialCompensationRequest request) async {
    ActionResponse? actionResponse = await _companyManager.updateFinancialCompany(request);
  ...
  Future<DataModel> createDeliveryPrice(DeliveryCompanyFinancialRequest request) async {
    ActionResponse? actionResponse = await _companyManager.createDeliveryPrice(request);
  ...
  Future<DataModel> updateDeliveryPrice(DeliveryCompanyFinancialRequest request) async {
    ActionResponse? actionResponse = await _companyManager.updateDeliveryPrice(request);

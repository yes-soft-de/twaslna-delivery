...
class OrderWithoutPendingLoadedState extends States {
  ...
  OrderWithoutPendingLoadedState(this.screenState, this.orders) : super(screenState){
  ...
  List<Widget> getOrders(List<OrderModel> orders) {
            ...
            Expanded(
              child: Container(
                ...
                child: ListTile(
                  ...
                  subtitle: Text(firstDate != null ? DateFormat('yyyy/M/d').format(firstDate!) : '0000/00/00'),
                ),
              ),
            ),
            Padding(
              padding: const EdgeInsets.all(8.0),
              child: Container(width: 32,height: 2.5,color: Theme.of(screenState.context).backgroundColor,),
            ),
            Expanded(
              child: Container(
                decoration: BoxDecoration(
                ...
                child: ListTile(
                  ...
                  subtitle: Text(lastDate != null ? DateFormat('yyyy/M/dd').format(lastDate!) : '0000/00/00'),
                ),
              ),
            ),
          ],
    ...
    ordersCard.add(
        Padding(
          ...
          child: ElevatedButton(
            ...
            onPressed:firstDate != null && lastDate != null ? () => screenState.getOrderFilteredDate(firstDate!, lastDate!): null,
            child: Padding(
    ...
    ordersCard.add(Padding(
          ...
          child: Divider(color: Theme.of(screenState.context).disabledColor.withOpacity(0.1),thickness: 2.0,),));

    orders.forEach((element) {
      ordersCard.add(OrderCard(
        onTap: (){
          Navigator.of(screenState.context).pushNamed(OrdersRoutes.ORDER_TIME_LINE,arguments: element.orderId);
        },

...
class PaymentsFromCaptainStateManager {
  ...
  void getCaptainPaymentsDetails(PaymentsFromCaptainScreenState screenState,int captainId){
    ...
    Future.wait([
      ...
      if (value[0].hasError || value[1].hasError){
        _stateSubject.add(PaymentsForCaptainLoadedState(screenState,null,null,error:value[0].error ?? value[1].error,));
      }
      else if (value[0].isEmpty || value[1].isEmpty) {
        _stateSubject.add(PaymentsForCaptainLoadedState(screenState,null,null,empty: value.isEmpty));
      }
      else {
         ProfileModel _model = value[0] as ProfileModel;
         BalanceModel _balance = value[1] as BalanceModel;
         _stateSubject.add(PaymentsForCaptainLoadedState(screenState,_model.data,_balance));
      }
    });
  }

  void makePayments(PaymentsFromCaptainScreenState screenState,CaptainPaymentsRequest request) {
    _stateSubject.add(LoadingState(screenState));

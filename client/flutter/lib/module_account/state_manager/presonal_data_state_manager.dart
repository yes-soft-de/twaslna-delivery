...
import 'package:twaslna_delivery/module_account/ui/state/personal_data/personal_data_loading_state.dart';
import 'package:twaslna_delivery/module_account/ui/state/personal_data/personal_data_state.dart';
import 'package:twaslna_delivery/module_upload/service/image_upload/image_upload_service.dart';
...
class PersonalDataStateManager {
  ...
  void postClientProfile(PersonalDataScreenState screenState , ProfileRequest request) {
  ...
  void uploadImage(String imagePath,PersonalDataScreenState screenState,ProfileModel profileModel){
    CustomFlushBarHelper.createSuccess(title:S.current.warnning, message:S.current.uploadingImagesPleaseWait,background: Theme.of(screenState.context).primaryColor,timeout: 1,top: true)..show(screenState.context);
    _uploadService.uploadImage(imagePath).then((value){
      if (value == null) {
        CustomFlushBarHelper.createError(title:S.current.warnning, message:S.current.uploadingImagesPleaseWait)..show(screenState.context);
        _stateSubject.add(PersonalDataLoadedState(screenState,profileModel));
      }
      else {
        profileModel.image = value;
        CustomFlushBarHelper.createSuccess(title:S.current.warnning, message:S.current.uploadImageSuccess)..show(screenState.context);
        _stateSubject.add(PersonalDataLoadedState(screenState,profileModel));
      }
    });
  }

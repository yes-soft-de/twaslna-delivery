// GENERATED CODE - DO NOT MODIFY BY HAND

// **************************************************************************
// InjectableConfigGenerator
// **************************************************************************

import 'package:get_it/get_it.dart' as _i1;
import 'package:injectable/injectable.dart' as _i2;

import '../main.dart' as _i78;
import '../module_account/account_module.dart' as _i22;
import '../module_account/manager/account_manager.dart' as _i21;
import '../module_account/repository/account_repository.dart' as _i3;
import '../module_account/service/account_service.dart' as _i23;
import '../module_account/state_manager/account_state_manager.dart' as _i24;
import '../module_account/ui/screen/account_screen.dart' as _i4;
import '../module_account/ui/screen/presonal_data_screen.dart' as _i17;
import '../module_auth/authoriazation_module.dart' as _i71;
import '../module_auth/manager/auth_manager/auth_manager.dart' as _i40;
import '../module_auth/presistance/auth_prefs_helper.dart' as _i5;
import '../module_auth/repository/auth/auth_repository.dart' as _i27;
import '../module_auth/service/auth_service/auth_service.dart' as _i41;
import '../module_auth/service/auth_service_without_fire_base/auth_service_api.dart'
    as _i42;
import '../module_auth/state_manager/login_state_manager/login_state_manager.dart'
    as _i48;
import '../module_auth/state_manager/register_state_manager/register_state_manager.dart'
    as _i50;
import '../module_auth/ui/screen/login_screen/login_screen.dart' as _i63;
import '../module_auth/ui/screen/register_screen/register_screen.dart' as _i64;
import '../module_chat/bloc/chat_page/chat_page.bloc.dart' as _i72;
import '../module_chat/chat_module.dart' as _i77;
import '../module_chat/manager/chat/chat_manager.dart' as _i59;
import '../module_chat/repository/chat/chat_repository.dart' as _i43;
import '../module_chat/service/chat/char_service.dart' as _i60;
import '../module_chat/ui/screens/chat_page/chat_page.dart' as _i76;
import '../module_home/home_module.dart' as _i73;
import '../module_home/manager/home_manager.dart' as _i44;
import '../module_home/repository/home_repository.dart' as _i28;
import '../module_home/service/home_service.dart' as _i45;
import '../module_home/state_manager/home_state_manager.dart' as _i46;
import '../module_home/ui/screen/home_screen.dart' as _i62;
import '../module_localization/presistance/localization_preferences_helper/localization_preferences_helper.dart'
    as _i7;
import '../module_localization/service/localization_service/localization_service.dart'
    as _i8;
import '../module_main/main_module.dart' as _i29;
import '../module_main/ui/screen/main_screen.dart' as _i10;
import '../module_my_notifications/manager/my_notifications_manager.dart'
    as _i30;
import '../module_my_notifications/my_notifications_module.dart' as _i31;
import '../module_my_notifications/repository/my_notifications_repository.dart'
    as _i11;
import '../module_my_notifications/service/my_notification_service.dart'
    as _i32;
import '../module_my_notifications/state_manager/my_notifications_state_manager.dart'
    as _i33;
import '../module_my_notifications/ui/screen/my_notifications_screen.dart'
    as _i12;
import '../module_network/http_client/http_client.dart' as _i25;
import '../module_notifications/preferences/notification_preferences/notification_preferences.dart'
    as _i15;
import '../module_notifications/repository/notification_repo.dart' as _i49;
import '../module_notifications/service/fire_notification_service/fire_notification_service.dart'
    as _i61;
import '../module_notifications/service/local_notification_service/local_notification_service.dart'
    as _i6;
import '../module_orders/manager/my_orders_manager.dart' as _i34;
import '../module_orders/orders_module.dart' as _i16;
import '../module_orders/repository/my_orders_repository.dart' as _i13;
import '../module_orders/service/my_orders_service.dart' as _i35;
import '../module_orders/state_manager/my_orders_state_manager.dart' as _i36;
import '../module_orders/ui/screen/my_orders_screen.dart' as _i14;
import '../module_report/manager/report_manager.dart' as _i65;
import '../module_report/presistance/report_prefs_helper.dart' as _i18;
import '../module_report/repository/report_repository.dart' as _i51;
import '../module_report/service/report_service.dart' as _i66;
import '../module_settings/settings_module.dart' as _i74;
import '../module_settings/ui/settings_page/settings_page.dart' as _i67;
import '../module_splash/splash_module.dart' as _i68;
import '../module_splash/ui/screen/splash_screen.dart' as _i52;
import '../module_stores/manager/store_list_manager.dart' as _i53;
import '../module_stores/manager/store_products.dart' as _i56;
import '../module_stores/repository/store_list_repository.dart' as _i37;
import '../module_stores/repository/store_products_repository.dart' as _i38;
import '../module_stores/service/store_list_service.dart' as _i54;
import '../module_stores/service/store_products_service.dart' as _i57;
import '../module_stores/state_manager/store_list_state_manager.dart' as _i55;
import '../module_stores/state_manager/store_products_state_manager.dart'
    as _i58;
import '../module_stores/store_module.dart' as _i75;
import '../module_stores/ui/screen/store_list_screen.dart' as _i69;
import '../module_stores/ui/screen/store_products_screen.dart' as _i70;
import '../module_theme/pressistance/theme_preferences_helper.dart' as _i19;
import '../module_theme/service/theme_service/theme_service.dart' as _i26;
import '../module_upload/manager/upload_manager/upload_manager.dart' as _i39;
import '../module_upload/repository/upload_repository/upload_repository.dart'
    as _i20;
import '../module_upload/service/image_upload/image_upload_service.dart'
    as _i47;
import '../utils/logger/logger.dart'
    as _i9; // ignore_for_file: unnecessary_lambdas

// ignore_for_file: lines_longer_than_80_chars
/// initializes the registration of provided dependencies inside of [GetIt]
_i1.GetIt $initGetIt(_i1.GetIt get,
    {String? environment, _i2.EnvironmentFilter? environmentFilter}) {
  final gh = _i2.GetItHelper(get, environment, environmentFilter);
  gh.factory<_i3.AccountRepository>(() => _i3.AccountRepository());
  gh.factory<_i4.AccountScreen>(() => _i4.AccountScreen());
  gh.factory<_i5.AuthPrefsHelper>(() => _i5.AuthPrefsHelper());
  gh.factory<_i6.LocalNotificationService>(
      () => _i6.LocalNotificationService());
  gh.factory<_i7.LocalizationPreferencesHelper>(
      () => _i7.LocalizationPreferencesHelper());
  gh.factory<_i8.LocalizationService>(
      () => _i8.LocalizationService(get<_i7.LocalizationPreferencesHelper>()));
  gh.factory<_i9.Logger>(() => _i9.Logger());
  gh.factory<_i10.MainScreen>(() => _i10.MainScreen());
  gh.factory<_i11.MyNotificationsRepository>(
      () => _i11.MyNotificationsRepository());
  gh.factory<_i12.MyNotificationsScreen>(() => _i12.MyNotificationsScreen());
  gh.factory<_i13.MyOrdersRepository>(() => _i13.MyOrdersRepository());
  gh.factory<_i14.MyOrdersScreen>(() => _i14.MyOrdersScreen());
  gh.factory<_i15.NotificationsPrefsHelper>(
      () => _i15.NotificationsPrefsHelper());
  gh.factory<_i16.OrdersModule>(
      () => _i16.OrdersModule(get<_i14.MyOrdersScreen>()));
  gh.factory<_i17.PersonalDataScreen>(() => _i17.PersonalDataScreen());
  gh.factory<_i18.ReportPrefsHelper>(() => _i18.ReportPrefsHelper());
  gh.factory<_i19.ThemePreferencesHelper>(() => _i19.ThemePreferencesHelper());
  gh.factory<_i20.UploadRepository>(() => _i20.UploadRepository());
  gh.factory<_i21.AccountManager>(
      () => _i21.AccountManager(get<_i3.AccountRepository>()));
  gh.factory<_i22.AccountModule>(() => _i22.AccountModule(
      get<_i4.AccountScreen>(), get<_i17.PersonalDataScreen>()));
  gh.factory<_i23.AccountService>(
      () => _i23.AccountService(get<_i21.AccountManager>()));
  gh.factory<_i24.AccountStateManager>(
      () => _i24.AccountStateManager(get<_i23.AccountService>()));
  gh.factory<_i25.ApiClient>(() => _i25.ApiClient(get<_i9.Logger>()));
  gh.factory<_i26.AppThemeDataService>(
      () => _i26.AppThemeDataService(get<_i19.ThemePreferencesHelper>()));
  gh.factory<_i27.AuthRepository>(
      () => _i27.AuthRepository(get<_i25.ApiClient>()));
  gh.factory<_i28.HomeRepository>(
      () => _i28.HomeRepository(get<_i25.ApiClient>()));
  gh.factory<_i29.MainModule>(() => _i29.MainModule(get<_i10.MainScreen>()));
  gh.factory<_i30.MyNotificationsManager>(
      () => _i30.MyNotificationsManager(get<_i11.MyNotificationsRepository>()));
  gh.factory<_i31.MyNotificationsModule>(
      () => _i31.MyNotificationsModule(get<_i12.MyNotificationsScreen>()));
  gh.factory<_i32.MyNotificationsService>(
      () => _i32.MyNotificationsService(get<_i30.MyNotificationsManager>()));
  gh.factory<_i33.MyNotificationsStateManager>(() =>
      _i33.MyNotificationsStateManager(get<_i32.MyNotificationsService>()));
  gh.factory<_i34.MyOrdersManager>(
      () => _i34.MyOrdersManager(get<_i13.MyOrdersRepository>()));
  gh.factory<_i35.MyOrdersService>(
      () => _i35.MyOrdersService(get<_i34.MyOrdersManager>()));
  gh.factory<_i36.MyOrdersStateManager>(
      () => _i36.MyOrdersStateManager(get<_i35.MyOrdersService>()));
  gh.factory<_i37.StoreListRepository>(
      () => _i37.StoreListRepository(get<_i25.ApiClient>()));
  gh.factory<_i38.StoreProductsRepository>(
      () => _i38.StoreProductsRepository(get<_i25.ApiClient>()));
  gh.factory<_i39.UploadManager>(
      () => _i39.UploadManager(get<_i20.UploadRepository>()));
  gh.factory<_i40.AuthManager>(
      () => _i40.AuthManager(get<_i27.AuthRepository>()));
  gh.factory<_i41.AuthService>(() =>
      _i41.AuthService(get<_i5.AuthPrefsHelper>(), get<_i40.AuthManager>()));
  gh.factory<_i42.AuthServiceApi>(() =>
      _i42.AuthServiceApi(get<_i5.AuthPrefsHelper>(), get<_i40.AuthManager>()));
  gh.factory<_i43.ChatRepository>(() =>
      _i43.ChatRepository(get<_i25.ApiClient>(), get<_i41.AuthService>()));
  gh.factory<_i44.HomeManager>(
      () => _i44.HomeManager(get<_i28.HomeRepository>()));
  gh.factory<_i45.HomeService>(() => _i45.HomeService(get<_i44.HomeManager>()));
  gh.factory<_i46.HomeStateManager>(
      () => _i46.HomeStateManager(get<_i45.HomeService>()));
  gh.factory<_i47.ImageUploadService>(
      () => _i47.ImageUploadService(get<_i39.UploadManager>()));
  gh.factory<_i48.LoginStateManager>(
      () => _i48.LoginStateManager(get<_i41.AuthService>()));
  gh.factory<_i49.NotificationRepo>(() =>
      _i49.NotificationRepo(get<_i25.ApiClient>(), get<_i41.AuthService>()));
  gh.factory<_i50.RegisterStateManager>(
      () => _i50.RegisterStateManager(get<_i41.AuthService>()));
  gh.factory<_i51.ReportRepository>(() => _i51.ReportRepository(
      get<_i25.ApiClient>(),
      get<_i41.AuthService>(),
      get<_i18.ReportPrefsHelper>()));
  gh.factory<_i52.SplashScreen>(
      () => _i52.SplashScreen(get<_i41.AuthService>()));
  gh.factory<_i53.StoreListManager>(
      () => _i53.StoreListManager(get<_i37.StoreListRepository>()));
  gh.factory<_i54.StoreListService>(
      () => _i54.StoreListService(get<_i53.StoreListManager>()));
  gh.factory<_i55.StoreListStateManager>(
      () => _i55.StoreListStateManager(get<_i54.StoreListService>()));
  gh.factory<_i56.StoreProductsManager>(
      () => _i56.StoreProductsManager(get<_i38.StoreProductsRepository>()));
  gh.factory<_i57.StoreProductsService>(
      () => _i57.StoreProductsService(get<_i56.StoreProductsManager>()));
  gh.factory<_i58.StoreProductsStateManager>(
      () => _i58.StoreProductsStateManager(get<_i57.StoreProductsService>()));
  gh.factory<_i59.ChatManager>(
      () => _i59.ChatManager(get<_i43.ChatRepository>()));
  gh.factory<_i60.ChatService>(() => _i60.ChatService(get<_i59.ChatManager>()));
  gh.factory<_i61.FireNotificationService>(() => _i61.FireNotificationService(
      get<_i15.NotificationsPrefsHelper>(), get<_i49.NotificationRepo>()));
  gh.factory<_i62.HomeScreen>(
      () => _i62.HomeScreen(get<_i46.HomeStateManager>()));
  gh.factory<_i63.LoginScreen>(
      () => _i63.LoginScreen(get<_i48.LoginStateManager>()));
  gh.factory<_i64.RegisterScreen>(
      () => _i64.RegisterScreen(get<_i50.RegisterStateManager>()));
  gh.factory<_i65.ReportManager>(
      () => _i65.ReportManager(get<_i51.ReportRepository>()));
  gh.factory<_i66.ReportService>(
      () => _i66.ReportService(get<_i65.ReportManager>()));
  gh.factory<_i67.SettingsScreen>(() => _i67.SettingsScreen(
      get<_i41.AuthService>(),
      get<_i8.LocalizationService>(),
      get<_i26.AppThemeDataService>(),
      get<_i61.FireNotificationService>()));
  gh.factory<_i68.SplashModule>(
      () => _i68.SplashModule(get<_i52.SplashScreen>()));
  gh.factory<_i69.StoreListScreen>(
      () => _i69.StoreListScreen(get<_i55.StoreListStateManager>()));
  gh.factory<_i70.StoreProductsScreen>(
      () => _i70.StoreProductsScreen(get<_i58.StoreProductsStateManager>()));
  gh.factory<_i71.AuthorizationModule>(() => _i71.AuthorizationModule(
      get<_i63.LoginScreen>(), get<_i64.RegisterScreen>()));
  gh.factory<_i72.ChatPageBloc>(
      () => _i72.ChatPageBloc(get<_i60.ChatService>()));
  gh.factory<_i73.HomeModule>(() => _i73.HomeModule(get<_i62.HomeScreen>()));
  gh.factory<_i74.SettingsModule>(
      () => _i74.SettingsModule(get<_i67.SettingsScreen>()));
  gh.factory<_i75.StoreModule>(() => _i75.StoreModule(
      get<_i69.StoreListScreen>(), get<_i70.StoreProductsScreen>()));
  gh.factory<_i76.ChatPage>(() =>
      _i76.ChatPage(get<_i72.ChatPageBloc>(), get<_i47.ImageUploadService>()));
  gh.factory<_i77.ChatModule>(
      () => _i77.ChatModule(get<_i76.ChatPage>(), get<_i41.AuthService>()));
  gh.factory<_i78.MyApp>(() => _i78.MyApp(
      get<_i26.AppThemeDataService>(),
      get<_i8.LocalizationService>(),
      get<_i61.FireNotificationService>(),
      get<_i6.LocalNotificationService>(),
      get<_i68.SplashModule>(),
      get<_i71.AuthorizationModule>(),
      get<_i77.ChatModule>(),
      get<_i74.SettingsModule>(),
      get<_i73.HomeModule>(),
      get<_i29.MainModule>(),
      get<_i75.StoreModule>(),
      get<_i31.MyNotificationsModule>(),
      get<_i22.AccountModule>()));
  return get;
}
